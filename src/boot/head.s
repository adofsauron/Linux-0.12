...
startup_32:
	...
	call setup_idt
	call setup_gdt
	mov $0x10, %eax				# reload all the segment registers
...
1:	inc %eax					# check that A20 really IS enabled
	mov %eax, 0x000000			# loop forever if it isn't
	cmp %eax, 0x100000
	je 1b
...
check_x87:
	...
	mov %cr0, %eax
	xor $6, %eax				/* reset MP, set EM */
	mov %eax, %cr0
	ret
.align 4
1:	fsetpm						/* fsetpm for 287, ignored by 387 */
	ret
...
.align 4
setup_paging:
	mov $1024*5, %ecx			/* 5 pages - pg_dir+4 page tables */
	xor %eax, %eax
	...
	mov $pg3+4092, %edi
	mov $0xfff007, %eax			/*  16Mb - 4096 + 7 (r/w user,p) */
	std
