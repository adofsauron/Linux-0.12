...
static unsigned int video_num_columns; /* Number of text columns	*/
static unsigned int video_mem_base; /* Base of video memory		*/
static unsigned int video_mem_term; /* End of video memory		*/
static unsigned int video_size_row; /* Bytes per row		*/
static unsigned int video_num_lines; /* Number of test lines		*/
...
static struct
{
	...
	unsigned int vc_restate;
	unsigned int vc_checkin;
...
void csi_m(int currcons)
{
	...
	for (i = 0; i <= npar; i++)
		switch (par[i])
		{
			case 0: attr = def_attr;
				break; /* default */
			case 1: attr = (iscolor ? attr | 0x08:attr | 0x0f);
				break; /* bold */
			...
			case 5: attr = attr | 0x80;
				break; /* blinking */
			case 7: attr = (attr << 4) | (attr >> 4);
				break; /* negative */
			case 22: attr = attr & 0xf7;
				break; /* not bold */
			case 24: attr = attr & 0xfe;
				break; /* not underline */
			case 25: attr = attr & 0x7f;
				break; /* not blinking */
			case 27: attr = def_attr;
				break; /* positive image */
			case 39: attr = (attr & 0xf0) | (def_attr & 0x0f);
				break;
			case 49: attr = (attr & 0x0f) | (def_attr & 0xf0);
				break;
			default:
				if (!can_do_colour)
					break;
