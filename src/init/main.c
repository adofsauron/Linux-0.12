...
extern void mem_init(int start, int end);
extern int rd_init(int mem_start, int length);
extern int kernel_mktime(struct tm * tm);
...
static int buffer_memory_end = 0;
static int main_memory_start = 0;
static char term[32];
...
void main_init(void) /* This really IS void, no error here. */
{
