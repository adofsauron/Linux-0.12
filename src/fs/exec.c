...
static unsigned int * create_tables(char * p, int argc, int envc)
{
	...
	sp -= argc + 1;
	argv = sp;
	put_fs_int((unsigned int) envp, --sp);
	put_fs_int((unsigned int) argv, --sp);
	put_fs_int((unsigned int) argc, --sp);
	while (argc-- > 0)
	{
		put_fs_int((unsigned int) p, argv++);
		while (get_fs_byte(p++)) /* nothing */;
	}
	put_fs_int(0, argv);
	while (envc-- > 0)
	{
		put_fs_int((unsigned int) p, envp++);
		while (get_fs_byte(p++)) /* nothing */;
	}
	put_fs_int(0, envp);
	return sp;
...
static int count(char ** argv)
{
	...
	if (tmp = argv)
		while (get_fs_int((unsigned int *) (tmp++)))
			i++;
								  ...
								  unsigned int p, int from_kmem)
{
	...
	while (argc-- > 0)
	{
		if (from_kmem == 1)
			set_fs(new_fs);
		if (!(tmp = (char *) get_fs_int(((unsigned int *) argv) + argc)))
			panic("argc is wrong");
		if (from_kmem == 1)
			set_fs(old_fs);
