...
int sys_select(unsigned int *buffer)
{
	...
	mask = ~((~0) << get_fs_int(buffer++));
	inp = (fd_set *) get_fs_int(buffer++);
	outp = (fd_set *) get_fs_int(buffer++);
	exp = (fd_set *) get_fs_int(buffer++);
	tvp = (struct timeval *) get_fs_int(buffer);

	if (inp)
		in = mask & get_fs_int(inp);
	if (outp)
		out = mask & get_fs_int(outp);
	if (exp)
		ex = mask & get_fs_int(exp);
	timeout = 0xffffffff;
	if (tvp)
	{
		timeout = get_fs_int((unsigned int *) &tvp->tv_usec) / (1000000 / HZ);
		timeout += get_fs_int((unsigned int *) &tvp->tv_sec) * HZ;
		timeout += jiffies;
	}
	...
	{
		verify_area(inp, 4);
		put_fs_int(res_in, inp);
	}
	if (outp)
	{
		verify_area(outp, 4);
		put_fs_int(res_out, outp);
	}
	if (exp)
	{
		verify_area(exp, 4);
		put_fs_int(res_ex, exp);
	}
	if (tvp)
	{
		verify_area(tvp, sizeof (*tvp));
		put_fs_int(timeout / HZ, (unsigned int *) &tvp->tv_sec);
		timeout %= HZ;
		timeout *= (1000000 / HZ);
		put_fs_int(timeout, (unsigned int *) &tvp->tv_usec);
	}
